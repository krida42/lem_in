...
void print_lemin(t_lemin *lemin) {
	...
	for (int i = 0; i < lemin->nb_rooms; i++) {
		...
		printf("Room ID: %d, Name: %s, Coordinates: (%d, %d), Links to: ", room->id, room->name, room->x, room->y);
		if (!room->links) {
			printf("No links\n");
		} else {
...
void clean_quit_if_room_null(t_lemin *lemin, t_room *room, char *line) {
	if (!room) {
		free(line);
		ft_error("ERROR: Invalid start room definition.", lemin);
	}
}
...
void free_and_quit(t_lemin *lemin, t_room *room, char *line) {
	free(line);
	free_room(room);
	ft_error("ERROR, free_and_quit", lemin);
}
...
void clean_quit_if_room_name_already_used(t_lemin *lemin, t_room *room, char *line) {
	if (is_room_name_already_used(lemin, room->name)) {
		free(line);
		free_room(room);
		ft_error("ERROR: Room name already used", lemin);
	}
}
...
void integrate_link_to_rooms(t_lemin *lemin, int parent_room_id, int child_room_id) {
	if (parent_room_id < 0 || parent_room_id >= lemin->nb_rooms ||
		...
		ft_error("ERROR: Invalid room ID in link.", lemin);
	}
	int *child_id_ptr = malloc(sizeof(int));
	if (!child_id_ptr)
		ft_error("ERROR: Malloc failed for link ID.", lemin);
	*child_id_ptr = child_room_id;
	t_list *new_link = ft_lstnew(child_id_ptr);
	if (!new_link) {
		free(child_id_ptr);
		ft_error("ERROR: Malloc failed for link node.", lemin);
	}
...
enum ParsingState process_parsing_ants(t_lemin *lemin, char *line) {
	...
	if (ants == -1) {
		free(line);
		ft_error("ERROR: Invalid number of ants.", lemin);
	}
	lemin->nb_ants = ants;
...
enum ParsingState process_parsing_links(t_lemin *lemin, char *line) {
	...
	if (!link_rooms_name) {
		free(line);
		ft_error("ERROR: Invalid link definition.", lemin);
	}
	...
	if (room_parent_id == -1 || room_child_id == -1) {
		free(line);
		ft_error("ERROR: Link references unknown room.", lemin);
	}
